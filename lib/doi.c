...
digits assigned to organizations; the suffix is assigned by the publisher
and was designed to be flexible with publisher identification standards. */
int
is_doi( char *s )
{
	if ( string_pattern( s, "##.####/", 0 ) || string_pattern( s, "##.#####/", 0 ) || string_pattern( s, "##.######/", 0 ) ) return 0;
	if ( string_pattern( s, "doi:##.####/", 0 ) || string_pattern( s, "doi:##.#####/", 0 ) || string_pattern( s, "doi:##.######/", 0 ) ) return 4;
	if ( string_pattern( s, "doi: ##.####/", 0 ) || string_pattern( s, "doi: ##.#####/", 0 ) || string_pattern( s, "doi: ##.######/", 0 ) ) return 5;
	if ( string_pattern( s, "doi: DOI: ##.####/", 0 ) || string_pattern( s, "doi: DOI: ##.#####/", 0 ) || string_pattern( s, "doi: DOI: ##.######/", 0 ) ) return 10;
	return -1;
}
